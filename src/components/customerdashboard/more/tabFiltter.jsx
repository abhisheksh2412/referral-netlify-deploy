...
function TabFiltter({ inputValue, inputOnChange, tabs, active, setActive }) {
  return (
    <div>
      <div className="flex items-center justify-between bg-white p-4 rounded-md mobile:inline-block mobile:w-full mb-8 ">
        <div>
          <div className="relative h-10 w-full min-w-[300px]  sm:min-w-[200px]">
        ...
        <div className="flex gap-2 mobile:gap-2 mobile:inline-block mobile:text-center mobile:grid mobile:grid-cols-2 mobile:mt-4">
          {tabs?.map((item, index) => (
            <button
              onClick={() => setActive(item.id)}
              key={index}
              className={clsx("relative px-8 mobile:px-3 py-3 text-sm  font-normal text-center text-white rounded-full bg-[#0e0a38]", active === item.id && "bg-blush-red")}
            >
              {item?.name}
            </button>
          ))}
