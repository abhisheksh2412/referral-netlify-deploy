...
function BuyPlasticCardForm() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      await dispatch(PlasticCardCheckout(data));
  ...
  const handlePayment = useCallback(async () => {
    ...
    if (status === "success") {
      ...
      await dispatch(
        PlasticCardSuccessApi(data, cardFormData, () =>
  ...
  const handleSelectQuantityAndMethod = useCallback(async () => {
    if (formik.values.number_of_cards && formik.values.shipping_method) {
      ...
      await dispatch(GetCardOrderTotal(data));
    }
    ...
    <Loader isLoading={isLoading || partner.isLoading || checkOut.isLoading}>
      <div>
        <form onSubmit={formik.handleSubmit}>
          ...
          <div className="mt-5 w-full bg-gray-100 p-4 rounded-lg">
            ...
            <div className="flex items-center">
              ...
              <div className="w-11/12">
                <p>
                  {`${plasticCardData?.store?.name || ""} , ${
                    plasticCardData?.store?.street || ""
                  } ,${plasticCardData?.store?.town || ""} \n ${
                    plasticCardData?.store?.mobile_number || ""
                  } \n ${plasticCardData?.store?.postal_code || ""}`}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className=" flex justify-between items-center gap-5 mt-5 w-full  rounded-lg">
            <h3 className="text-base font-semibold">
              Total Price :{" "}
